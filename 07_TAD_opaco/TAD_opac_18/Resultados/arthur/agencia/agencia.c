...
{
    int numContas, numTransacoes;
    int idConta;
    float valorOperacao;
    tConta *contaEncontrada;

    // Leitura do número de contas
    if (scanf("%d", &numContas) != 1)
        return; // [cite: 12]

    // Leitura das contas
    for (int i = 0; i < numContas; i++)
    {
        if (scanf("%d", &idConta) != 1)
            return; // [cite: 13]
        if (idConta > 0)
        { // [cite: 5, 15]
    ...
    if (scanf("%d", &numTransacoes) != 1)
        return; // [cite: 14]

    // Leitura das transações
    for (int i = 0; i < numTransacoes; i++)
    {
        if (scanf("%d %f", &idConta, &valorOperacao) != 2)
            return; // [cite: 14]

        // Confere se a conta existe na agência [cite: 15]
        contaEncontrada = BuscaContaPorId(agencia, idConta);

        if (contaEncontrada != NULL)
        { // [cite: 15]
            tOperacao *novaOperacao = CriaOperacao(contaEncontrada, valorOperacao);
